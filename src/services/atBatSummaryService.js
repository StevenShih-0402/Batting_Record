...
export const getAtBatHistory = (userId, setRecordsCallback, setLoadingCallback) => {
    ...
    const q = query(
        summaryRef,
        where("userId", "==", userId)
        // orderBy("startAt", "desc") // [FIXED] 避免複合索引需求，改由前端排序
    );
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
        ...
        // 前端排序：依照 startAt (最新的在前面)
        historyData.sort((a, b) => {
            const timeA = a.startAt?.toMillis?.() || a.startAt?.getTime?.() || 0;
            const timeB = b.startAt?.toMillis?.() || b.startAt?.getTime?.() || 0;
            return timeB - timeA;
        });
